...
class AddPayment extends StatefulWidget {
  final PayReceiveModel? updatePayReceive;
  final String transaction;
  const AddPayment({Key? key, this.updatePayReceive, required this.transaction})
      : super(key: key);
...
class _AddPaymentState extends State<AddPayment> {
  ...
  String buttonText = 'Make Payment';
  ...
  String transaction = AppConstants.paymentTransaction;

  save() async {
    var batch = db.batch();
    if (_formKey.currentState!.validate()) {
      String pid = widget.updatePayReceive == null
          ? randomAlphaNumeric(8)
          : widget.updatePayReceive!.payRecId!;
      String ledgerCreditID = widget.updatePayReceive == null
          ? randomAlphaNumeric(10)
          : widget.updatePayReceive!.creditDebitId!;
      ...
      double creditAmt = 0;
      double debitAmt = 0;
      if (transaction == AppConstants.paymentTransaction) {
        print("in If");
        creditAmt = double.tryParse(amountController.text)!;
      } else {
        print("else");

        debitAmt = double.tryParse(amountController.text)!;
      }
      print(transaction);
      print("$creditAmt credit $debitAmt debit");
      ...
      PayReceiveModel payReceive = PayReceiveModel(
          ...
          creditDebitId: ledgerCreditID,
          createdAt: widget.updatePayReceive == null
              ? DateTime.now()
              : widget.updatePayReceive!.paymentDate);
      ...
      batch.set(db.collection(transaction).doc(pid), payReceive.toJson());
      batch.set(
          db.collection('Ledger').doc(ledgerCreditID), ledgerCredit.toJson());
      batch.commit().then((value) {
        return Flushbar(
          title: "Success!!",
          message: transaction == AppConstants.paymentTransaction
              ? "Payment of Rs.${amountController.text} has been done successfully!!"
              : "Rs.${amountController.text} received successfully!!",
  ...
  String selectedPaymentDate = DateFormat('yyyy-MM-dd').format(DateTime.now());
  ...
  checkOptions() async {
    if (widget.updatePayReceive != null) {
      buttonText = 'Update ${widget.transaction}';
      labelText = 'Update ${widget.transaction}';
      status = 'updated';
      selectedPaymentDate =
          DateFormat('yyyy-MM-dd').format(widget.updatePayReceive!.paymentDate);
      pidController.text = widget.updatePayReceive!.paymentNo;
      selectedPartyController.text =
          widget.updatePayReceive!.partyId.partyName!;

      selectedPartyModel = partyModelMenu.firstWhere(
          (element) => element.pid == widget.updatePayReceive!.partyId.pid);
      amountController.text = (widget.updatePayReceive!.creditAmount +
              widget.updatePayReceive!.debitAmount)
          .toString();
      // _paymentType =
      //     widget.updatePayReceive!.transactionType;
      narrationController.text = widget.updatePayReceive!.narration!;
    } else {
      buttonText = 'Confirm ${widget.transaction}';
      labelText = widget.transaction;
    }
